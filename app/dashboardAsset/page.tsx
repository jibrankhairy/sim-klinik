...
const DonutChartCard = ({ title, data, summaryText }: { title: string, data: any, summaryText: React.ReactNode }) => (
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="w-32 h-32">
                 <svg className="w-full h-full" viewBox="0 0 36 36">
                    ...
                    <circle cx="18" cy="18" r="15.915" fill="none" stroke="#e6e6e6" strokeWidth="3"/>
                    <circle cx="18" cy="18" r="15.915" fill="none" stroke="#fdba74" strokeWidth="3" strokeDasharray={`${data.percent1}, ${100-data.percent1}`}/>
                    <circle cx="18" cy="18" r="15.915" fill="none" stroke="#60a5fa" strokeWidth="3" strokeDasharray={`${data.percent2}, ${100-data.percent2}`} strokeDashoffset={`-${data.percent1}`}/>
                </svg>
            </div>
...
const LineChartCard = ({ title }: { title: string }) => (
    <div className="bg-white p-6 rounded-lg shadow-md">
        <h3 className="text-lg font-bold text-gray-800 mb-4">{title}</h3>
        <div className="h-60 bg-gray-50 rounded-md flex items-center justify-center"><p className="text-gray-400">Line Chart Placeholder</p></div>
    </div>
);
...
const CalendarCard = () => (
    <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-7 gap-2 mt-4 text-center text-sm">
            ...
            <div className="text-gray-400">31</div><div>1</div><div>2</div><div>3</div><div>4</div><div>5</div><div>6</div>
            <div>7</div><div>8</div><div>9</div><div>10</div><div className="bg-blue-500 text-white rounded-full">11</div><div>12</div><div>13</div>
        </div>
    </div>
);
...
export default function AssetDashboardPage() {
    ...
    const valuePercent = summary.totalInitialValue > 0 ? (summary.totalCurrentValue / summary.totalInitialValue) * 100 : 0;

    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <DonutChartCard title="Aset Aktif" data={{ percent1: 70, percent2: 30 }} summaryText={<p>Ringkasan Aset Aktif</p>} />
                <DonutChartCard title="Nilai Aset" data={{ percent1: valuePercent, percent2: 100 - valuePercent }} summaryText={<><p>Nilai Awal: {formatRupiah(summary.totalInitialValue)}</p><p>Nilai Buku: {formatRupiah(summary.totalCurrentValue)}</p><p>Penyusutan: {formatRupiah(summary.totalDepreciation)}</p></>} />
            </div>
